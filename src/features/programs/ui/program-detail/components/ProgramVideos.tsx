...
export function ProgramVideos({ videos }: ProgramVideosProps) {
  const [selectedVideoId, setSelectedVideoId] = useState<number | null>(videos[0]?.id ?? null)
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
      ...
      <div className="space-y-3">
        ...
        {videos.map((video, index) => {
          const { isLocked } = getVideoAccessState(video.is_free)
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isLocked && "border-amber-200/80 bg-amber-50/50 hover:border-amber-200",
              )}
            >
            <div className="flex items-center gap-4">
              ...
              >
                {isLocked ? (
                  ...
                  <PlayCircle className={cn("w-6 h-6", isSelected ? "text-white" : "text-gray-600")} />
                )}
              </div>
