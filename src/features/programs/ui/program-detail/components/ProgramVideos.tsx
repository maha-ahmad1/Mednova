  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-2xl font-bold text-gray-900">محتوى البرنامج</h2>
        <span className="text-sm text-gray-500">إجمالي الفيديوهات: {videos.length}</span>
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-lg font-bold text-gray-900">قائمة الفيديوهات</h3>
          ...
          {videos.map((video, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                >
                  <PlayCircle
                    className={`w-6 h-6 ${selectedVideo?.id === video.id ? "text-white" : "text-gray-600"}`}
                  />
                </div>
