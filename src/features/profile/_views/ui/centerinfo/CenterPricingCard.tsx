...
export function CenterPricingCard({ profile, userId, refetch }: CenterPricingCardProps) {
  ...
  const getFieldError = (field: keyof typeof values) => {
    ...
    const clientError = pricingSchema.shape[field]?.safeParse(values[field] ?? "").error?.issues?.[0]?.message
        ...
        <div className="bg-gray-50/50 p-6 rounded-xl border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormInput
              ...
              onChange={(e) => setValues((v) => ({ ...v, video_consultation_price: e.target.value }))}
              rtl={isRtl}
              className="no-spinner bg-white"
              error={getFieldError("video_consultation_price")}
              placeholder={t("pricingCard.videoPricePlaceholder")}
            />
            ...
            <FormInput
              ...
              onChange={(e) => setValues((v) => ({ ...v, chat_consultation_price: e.target.value }))}
              rtl={isRtl}
              className="no-spinner bg-white"
              error={getFieldError("chat_consultation_price")}
              placeholder={t("pricingCard.chatPricePlaceholder")}
            />
