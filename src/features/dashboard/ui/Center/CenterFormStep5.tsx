...
import { useCenterStore } from "@/features/dashboard/hooks/useCenterStore"
...
export function CenterFormStep5({ onBack, formData, updateFormData, setGlobalErrors }: CenterStep5Props) {
  ...
  const onSubmit: SubmitHandler<Step5Data> = async (data) => {
    ...
    try {
      ...
      const rawPayload = {
        ...
        birth_date: typeof formData.birth_date === "string" ? formData.birth_date : undefined,
        image: formData.image instanceof File ? formData.image : z.string().parse(formData.image),
        specialty_id: Array.isArray(formData.specialty_id) ? formData.specialty_id.map(String) : [],
        year_establishment: typeof formData.year_establishment === "string" ? formData.year_establishment : undefined,
        has_commercial_registration:
          formData.has_commercial_registration === 1 || formData.has_commercial_registration === true,
        ...
        commercial_registration_file:
          formData.commercial_registration_file instanceof File ? formData.commercial_registration_file : undefined,
        commercial_registration_authority:
          typeof formData.commercial_registration_authority === "string"
            ...
            : undefined,
        license_authority: typeof formData.license_authority === "string" ? formData.license_authority : undefined,
        license_file: formData.license_file instanceof File ? formData.license_file : undefined,
        license_number: typeof formData.license_number === "string" ? formData.license_number : undefined,
        bio: data.bio,
        ...
        formatted_address: typeof formData.formatted_address === "string" ? formData.formatted_address : undefined,
      } as Record<string, unknown>

      if (eveningFlag) {
        ;(rawPayload as Record<string, unknown>).start_time_evening = formData.start_time_evening || ""
        ;(rawPayload as Record<string, unknown>).end_time_evening = formData.end_time_evening || ""
      }
