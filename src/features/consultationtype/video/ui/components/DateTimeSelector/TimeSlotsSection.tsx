...
export default function TimeSlotsSection({ selectedDate, selectedTime, setSelectedTime, groupedSlots, isLoadingSlots, availableSlotsLength }: Props) {
  if (!selectedDate) {
    return (
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar pt-4">
      {groupedSlots.morning.length > 0 && (
        <TimeSlotGroup title="الفترة الصباحية" times={groupedSlots.morning} selectedTime={selectedTime} onSelect={setSelectedTime} />
      )}

      {groupedSlots.afternoon.length > 0 && (
        <TimeSlotGroup title="بعد الظهر" times={groupedSlots.afternoon} selectedTime={selectedTime} onSelect={setSelectedTime} />
      )}

      {groupedSlots.evening.length > 0 && (
        <TimeSlotGroup title="الفترة المسائية" times={groupedSlots.evening} selectedTime={selectedTime} onSelect={setSelectedTime} />
      )}
    </div>
  );
}
