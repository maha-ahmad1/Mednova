...
export default function BookingPage({ doctorId, patientId }: BookingPageProps) {
  ...
  if (logic.isPageLoading || logic.isLoadingProvider) return <BookingPageSkeleton />;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6 md:space-y-8">
            <DateTimeSelector
              selectedDate={logic.selectedDate}
              setSelectedDate={(d) => {
                logic.setSelectedDate(d);
                logic.setSelectedTime("");
              }}
              selectedTime={logic.selectedTime}
              setSelectedTime={logic.setSelectedTime}
              timeZone={logic.timeZone}
              setTimeZone={logic.setTimeZone}
              timeZones={logic.timeZones}
              groupedSlots={logic.groupedSlots}
              isLoadingSlots={logic.isLoadingSlots}
              availableSlotsLength={logic.availableSlots.length}
            />

            <BookingButton onClick={logic.handleBooking} disabled={!logic.selectedDate || !logic.selectedTime || logic.isSubmitting} isSubmitting={logic.isSubmitting} />
          </div>
        </div>
      </div>
    </div>
