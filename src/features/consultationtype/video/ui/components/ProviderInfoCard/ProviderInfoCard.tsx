...
export default function ProviderInfoCard({ provider, consultantType, selectedDate, selectedTime, onBack, isLoading }: Props) {
  const getSpecialty = () => {
    ...
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 p-4 sm:p-6">
      <div className="flex flex-col items-center text-center mb-4 sm:mb-6">
        <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-[#32A88D] to-[#2a8a7a] rounded-full flex items-center justify-center mb-3 sm:mb-4 overflow-hidden">
          <Image src={getProviderImage() || "/placeholder.svg"} alt={provider?.full_name || (consultantType === "therapist" ? "المختص" : "المركز")} width={80} height={80} className="w-full h-full object-cover" />
        </div>
      ...
      <div className="border-t border-gray-100 pt-4 sm:pt-6">
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100 text-xs sm:text-sm max-w-[60%] truncate">{getSpecialty()}</Badge>
          </div>
...
<div className="mt-4">
        <Button variant="outline" className="w-full border-[#32A88D] text-[#32A88D] rounded-xl py-5 sm:py-6 text-sm sm:text-base cursor-pointer hover:bg-[#32A88D]/10 hover:border-[#2a8a7a] hover:text-[#2a8a7a] transition-colors flex items-center justify-center" onClick={onBack}>
