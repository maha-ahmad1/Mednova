...
import { useConsultantWallet } from "@/features/financial/hooks";
import { formatLastUpdated } from "@/features/financial/utils/format";
...
export function WalletPageView() {
  ...
  const { data: wallet, isLoading } = useConsultantWallet();
  ...
  const lastUpdated = formatLastUpdated(wallet?.last_updated ?? "");
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-8 space-y-6" dir="rtl">
      ...
      {/* <button
        ...
        className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-[#32A88D] transition-colors"
      >
        {hidden ? (
        ...
        {hidden ? "إظهار الأرصدة" : "إخفاء الأرصدة"}
      </button> */}
